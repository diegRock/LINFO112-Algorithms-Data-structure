...
public class WordTransformationSP {
    ...
    public static int minimalCost(String from, String to) {
        ...
        pq.add(new Word(from, 0));

        while(!pq.isEmpty()){

            Word current = pq.poll();
            String current_string = current.str;

            for (int i = 0; i < current_string.length() - 1; i++) {
                for (int j = i + 2; j <= current_string.length(); j++) {

                    String sub_string = rotation(current_string, i, j);
                    int cost = current.cost + (j - i);

                    if(!distances.containsKey(sub_string) || distances.get(sub_string) > cost){
                        distances.put(sub_string, cost);
                        pq.add(new Word(sub_string, cost));
                    }
                }
            }
        }
