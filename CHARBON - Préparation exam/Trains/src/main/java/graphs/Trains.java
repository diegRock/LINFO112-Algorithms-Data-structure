...
public class Trains {
     ...
     */
    // Plus court chemin d'une station à toutes les autres stations !
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        ...
        pq.addAll(relations.keySet());

        while(!pq.isEmpty()){

            StationTime current = pq.poll();

            if(!distances.containsKey(current.station)) continue;
            if(current.time < distances.get(current.station)) continue;


            LinkedList<StationTime> voisins = relations.get(current);
            for(StationTime i : voisins){
                if(!distances.containsKey(i.station) || distances.get(i.station) > i.time) {
                    distances.put(i.station, i.time);
                }
            }
        }
        return distances;
