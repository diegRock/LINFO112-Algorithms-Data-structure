...
public class Trains {
    ...
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        ...
        while(!pq.isEmpty()){
            ...
            for(StationTime voisins : relations.get(current)){

                if(!distances.containsKey(voisins.station) || voisins.time < distances.get(voisins.station)){
                    distances.put(voisins.station, voisins.time);
                }
            }
        }
        return distances;
