...
public class MinMaxHeap<Key extends Comparable<Key>> {
    ...
    public Key max() {
        ...
        Key max_val;
         if(higherThan(content[2], content[3])){
             max_val = content[2];
         } else {
             max_val = content[3];
         }
    ...
    public void swim(int position) {
        ...
        for (int i = position; i > 1; i = i / 2) {
            ...
            boolean is_it_a_min_layer_man = getNodeDepth(i) % 2 == 0;

            int parent_index = i / 2;
            int gp_index = i / 4;
            boolean has_gp = gp_index > 0;

            if(is_it_a_min_layer_man){
                if(higherThan(content[i], content[parent_index])) swap(i, parent_index);
                else if(has_gp && lessThan(content[i], content[gp_index])) swap(i, gp_index);
            } else {
                if(lessThan(content[i], content[parent_index])) swap(i, parent_index);
                else if(has_gp && higherThan(content[i], content[gp_index])) swap(i, gp_index);
            }
        }
    }
