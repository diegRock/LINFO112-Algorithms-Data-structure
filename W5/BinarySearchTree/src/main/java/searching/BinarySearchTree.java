...
public class BinarySearchTree {
    ...
    public static Integer ceil(BSTNode<Integer> root, int value) {
        return ceilrec(root, value, null);
    }
    public static Integer ceilrec(BSTNode<Integer> root, int value, Integer candidate){
        if(value == root.key) return value;
        if(value > root.key){
            if(root.right == null) return candidate;
            return ceilrec(root.right, value, candidate);
        }

        if(root.left == null) return root.key;
        return ceilrec(root.left, value, root.key);
    }
