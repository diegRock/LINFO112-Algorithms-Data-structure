...
public class MinMaxHeap<Key extends Comparable<Key>> {
    ...
    public Key max() {
        ...
        if (content[2].compareTo(content[3]) > 0) {
            // content[2] plus grand que content [3]
            return content[2];
        } else {
            // L'inverse
            return content[3];
    ...
    public void swim(int position) {
        ...
        for(int i = position; i > 1; i = i / 2){
            ...
            boolean is_min_layer = getNodeDepth(i) % 2 == 0;

            int parent = i/2;
            int grand_parent = i/4;
            boolean has_gp = i/4 != 0;

            if(is_min_layer){
                if(lessThan(content[parent], content[i])) swap(parent, i);
                if(has_gp && higherThan(content[grand_parent], content[i])) swap(grand_parent, i);
            }
            if(higherThan(content[parent], content[i])) swap(parent, i);
            if(has_gp && lessThan(content[grand_parent], content[i])) swap(grand_parent, i);
        }
