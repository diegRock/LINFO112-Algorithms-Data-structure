...
public class BinarySearchTreeHigher<Key extends Comparable<Key>, Value> {
    ...
    // Attentiion ! !(root.left != null && root.right != null) --> !root.left != null || !root.right != null !!! Proposition logique
    // Il est donc préférable de faire une fonciton auxiliaire qui check si on est sur une leaf ou pas !
    ...
    private Key higherKey(Node x, Key key) {
        return higherKeyAux(root, key, null);
    }
    private Key higherKeyAux(Node x, Key key, Key prev){

        if(key.compareTo(x.key) >= 0){
            if(x.right == null) return prev;
            return higherKeyAux(x.right, key, prev);
        }

        if(x.left == null)  return x.key;
        return higherKeyAux(x.left, key, x.key);
