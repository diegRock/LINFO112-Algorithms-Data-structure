...
public class PreorderToBST {
    ...
    *                  ------ 7           12 ------
    *                                 
    *  PreOrder c'est key puis gauche key gauche droit
    */
    ...
    public PreorderToBST(int [] preOrderInput) {
        ...
        root = preorderRead(preOrderInput);
    }

    public Node preorderRead(int [] preOrderInput) {
        // !!!!!!! TODO !!!!!!!!
        if (preOrderInput.length == 0) return null;

        int start = preOrderInput[0];

        int mid = 1;
        while(mid < preOrderInput.length && preOrderInput[mid] < start) mid++;
        int [] leftArray = Arrays.copyOfRange(preOrderInput, 1, mid);
        int [] rightArray = Arrays.copyOfRange(preOrderInput, mid, preOrderInput.length);

        Node left;
        Node right;

        if(leftArray.length == 1){
            left = new Node(null, null, leftArray[0]);
        } else {
            left = preorderRead(leftArray);
        }

        if(rightArray.length == 1){
            right = new Node(null, null, rightArray[0]);
        } else {
            right = preorderRead(rightArray);
        }

        return new Node(left, right, start);
    }
