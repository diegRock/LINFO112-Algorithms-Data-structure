...
public class GlobalWarmingPaths {
    ...
    public List<Point> shortestPath(Point p1, Point p2) {
        ...
        int lenX = altitude.length;
        int lenY = altitude[0].length;
        ...
        Point start = p1;
        Point end = p2;
        if(altitude[start.x][start.y] <= waterLevel || altitude[end.x][end.y] <= waterLevel) return queue;
        queue.add(start);
        ...
        if(!marked[end.x][end.y]) return new LinkedList<>();

        LinkedList<Point> path = new LinkedList<>();
        Point iter = end;
        while(iter.x != start.x || iter.y != start.y){
            path.add(new Point(iter.x, iter.y));
