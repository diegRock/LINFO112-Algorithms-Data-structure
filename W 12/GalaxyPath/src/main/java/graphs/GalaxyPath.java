 ...
 * is it possible to reach a viable galaxy (any of them) and --> Plus court chemin parmis tous les noeuds donc dijkstra !
 * how long would it take at least knowing that each by-pass takes 1 year to be crossed ?
 */

// Each node is a galaxy  and the edges are the hyperspatial by-passes
...
public class GalaxyPath {
    ...
    public static int findPath(int [][] graph, int source, Set<Integer> destinations) {
        ...
        int lenXY = graph.length;
        boolean [] marked = new boolean[lenXY];
        int [] distances = new int[lenXY];
        ...
        while (!queue.isEmpty()){
            ...
            for (int j = 0; j < lenXY; j++) {
                if(!marked[j] && distances[i] < graph[i][j]){
                    queue.add(j);
                    marked[j] = true;
